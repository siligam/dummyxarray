...
def parse_ncdump_header(header_text: str) -> Dict[str, Any]:
    ...
    {'time': 10, 'lat': 64}
    """
    metadata = {"dimensions": {}, "variables": {}, "global_attrs": {}}
...
def _parse_variables(vars_text: str) -> Dict[str, Dict[str, Any]]:
    ...
    for line in vars_text.split("\n"):
        ...
        if var_match:
            ...
            current_var = name
            variables[name] = {"dims": dims, "dtype": dtype, "attrs": {}}
...
def from_ncdump_header(header_text: str, record_history: bool = True):
    ...
    for coord_name, coord_info in coords.items():
        ds.add_coord(coord_name, dims=coord_info["dims"], attrs=coord_info["attrs"])

    # Add variables
    for var_name, var_info in variables.items():
        ds.add_variable(var_name, dims=var_info["dims"], attrs=var_info["attrs"])
