...
def example_from_xarray_with_data():
    ...
    xr_ds = xr.Dataset(
        data_vars={"temperature": (["time"], np.array([273.15, 274.0, 275.5]))},
        coords={"time": (["time"], np.array([0, 1, 2]))},
...
def example_preserve_encoding():
    ...
    xr_ds = xr.Dataset(data_vars={"temperature": (["time"], np.random.rand(100))})

    # Set encoding
    xr_ds["temperature"].encoding = {"dtype": "float32", "chunks": (50,), "compressor": None}
