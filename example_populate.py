...
def example_basic_populate():
    ...
    print(f"  Temperature range: [{ds.variables['temperature'].data.min():.2f}, "
          f"{ds.variables['temperature'].data.max():.2f}] K")
    print(f"  Latitude range: [{ds.coords['lat'].data.min():.2f}, "
...
def example_different_variable_types():
    ...
    # Wind components
    ds.add_variable("ua", ["time"],
                   attrs={"standard_name": "eastward_wind", "units": "m s-1"})
    ds.add_variable("va", ["time"],
                   attrs={"standard_name": "northward_wind", "units": "m s-1"})
    
    # Relative humidity
    ds.add_variable("hur", ["time"],
                   attrs={"standard_name": "relative_humidity", "units": "%"})
    ...
    # Show statistics
    print("\nVariable Statistics:")
    print(f"  Temperature (tas):  {ds.variables['tas'].data.mean():.2f} K "
          f"[{ds.variables['tas'].data.min():.2f}, {ds.variables['tas'].data.max():.2f}]")
    print(f"  Precipitation (pr): {ds.variables['pr'].data.mean():.6f} kg m-2 s-1 "
          f"(all positive: {(ds.variables['pr'].data >= 0).all()})")
    print(f"  Pressure (psl):     {ds.variables['psl'].data.mean():.0f} Pa "
          f"[{ds.variables['psl'].data.min():.0f}, {ds.variables['psl'].data.max():.0f}]")
    print(f"  U-wind (ua):        {ds.variables['ua'].data.mean():.2f} m/s "
          f"[{ds.variables['ua'].data.min():.2f}, {ds.variables['ua'].data.max():.2f}]")
    print(f"  V-wind (va):        {ds.variables['va'].data.mean():.2f} m/s "
          f"[{ds.variables['va'].data.min():.2f}, {ds.variables['va'].data.max():.2f}]")
    print(f"  Humidity (hur):     {ds.variables['hur'].data.mean():.1f}% "
...
def example_from_yaml_then_populate():
    ...
    template.add_variable("tas", ["time", "lat", "lon"],
                         attrs={"standard_name": "air_temperature", "units": "K"})
    template.add_variable("pr", ["time", "lat", "lon"],
                         attrs={"standard_name": "precipitation_flux"})
